      ...
      <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 w-full sm:w-auto">
        <div class="relative w-full sm:w-72">
          <input
            ...
            class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm sm:text-base"
          >
          <svg class="pointer-events-none absolute left-3 top-2.5 h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
          </svg>
        </div>
        <button
          @click="showModal = true"
          class="inline-flex justify-center items-center bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg font-medium transition-colors text-sm sm:text-base"
        >
          + Agregar Producto
        </button>
      </div>
...
const products = ref([
  { id: 1, name: 'Ramo de Rosas Rojas', sku: 'RRB-001', stock: 45,  inTransit: 200, projectedSales: 350, seasonal: 'San Valentín Alta' },
  { id: 2, name: 'Disfraz Halloween - Bruja', sku: 'HCW-002', stock: 150, inTransit: 0,   projectedSales: 80,  seasonal: 'Pico Halloween' },
  { id: 3, name: 'Luces Árbol Navidad',      sku: 'CTL-003', stock: 850, inTransit: 300, projectedSales: 120, seasonal: 'Temporada Navideña' },
  { id: 4, name: 'Electrónicos Black Friday', sku: 'BFE-004', stock: 200, inTransit: 500, projectedSales: 450, seasonal: 'Pico Black Friday' }
])

const newProduct = ref({
  name: '', sku: '', stock: 0, projectedSales: 0
})
...
const addProduct = () => {
  const p = {
    ...
    inTransit: 0,
    projectedSales: newProduct.value.projectedSales || 0,
    seasonal: 'Normal'
  }
...
/* placeholder para botón Editar en cards/tabla */
function openEdit(p){ console.log('Editar', p) }
</script>
